import { MoodArtRequest, MoodArtResponse } from "@shared/types.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    const { mood, additionalDetails } = await req.json() as MoodArtRequest;
    ...
    const errorResponse: MoodArtResponse = {
      artDescription: "",
      error: error.message
    };
